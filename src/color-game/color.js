    ...
    try {
        ...
        if (timeResult.period != req.body.period) {
            ...
            if (req.body.method == 'Number') {
                ...
                await queryAsync("UPDATE `wallet` SET `wagering`= `wagering` + ?,`game_wallet`= `game_wallet` - ? WHERE `user_name`=?", [parseInt(req.body.total_amount), parseInt(req.body.total_amount), req.body.mobile]);
